...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 15 13:54:21 2023
...
def run_simul(inputs={}):
    ...
    if os.path.exists(tablefile):    
        ...
        crs.yaml(dni=950, sunshape=pm.sunshape, csr=pm.crs, half_angle_deg=pm.half_angle_deg, std_dev=pm.std_dev)
...
# rec_fr = 0.008 
# he_av_design = 0.99
# A_heliostat = W_helio*H_helio 
# Q_flow_des = P_gross / eff_blk
# R_des = SM * Q_flow_des / ((1 - rec_fr)*eta_rcv_des)
# A_field = R_des / (eff_opt * he_av_design * dni_des)
# n_helios = np.ceil(A_field / A_heliostat)
# Q_in_rcv = R_des;
# H_tower = 0.154 * np.sqrt(twr_ht_const * (A_field / (gnd_cvge * excl_fac)) /np.pi)
# A_receiver = A_field / C 
# W_rcv = np.sqrt(A_receiver / np.pi * AR_rec)
# H_rcv = W_rcv* AR_rec
# n_row_oelt = 51
# n_col_oelt = 25
# R1=40
...
if __name__=='__main__':
    ...
    field_type='surround'
    wea_file='/home/srspoke/SolarTherm/SolarTherm/Data/Weather/gen3p3_Daggett_TMY3_EES.motab'
    inputs={'casedir':case, 'lat':lat, 'dni_des':dni_des, 'Q_in_rcv':Q_in_rcv, 'W_rcv':W_rcv, 'H_rcv':H_rcv, 'H_tower':H_tower, 'wea_file':wea_file, 'n_row_oelt':n_row_oelt, 'n_col_oelt': n_col_oelt, 'rcv_type': 'cylinder', 'R1':R1, 'fb':fb, 'field_type': field_type,"n_W_rcv":n_W_rcv,"n_H_rcv":n_H_rcv, "n_rays":n_rays }
